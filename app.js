...
const seed = {
  properties: [
    {id:uid(),address:'ул. Атонели, Старый Тбилиси',type:'1 спальня / посуточно и долгосрочно',district:'Центр',price:850,status:'Свободно',agent:'Irakli',owner:'База собственников',phone:'995XXXXXXXXX',notes:'Итальянский двор, рядом рынок Орбелиани, резиденция президента, Дом юстиции, проспект Руставели.'},
    {id:uid(),address:'Кипшидзе 6, Ваке',type:'Коммерция 175 м²',district:'Ваке',price:3675,status:'В работе',agent:'Dima',owner:'База собственников',phone:'995XXXXXXXXX',notes:'Фасад 17 м, $21/м², рядом Nikora / GPS / PSP / универсам / зоомагазин, Vake Residence 100 м.'},
    {id:uid(),address:'ул. Аракишвили, Ваке',type:'2 спальни',district:'Ваке',price:1100,status:'Свободно',agent:'Misha',owner:'База собственников',phone:'995XXXXXXXXX',notes:'Арка со шлагбаумом, 2-й подъезд, 3-й этаж, коричневая дверь. Vake Nest / Falcon Nest.'},
    {id:uid(),address:'Outlook Ваке / The Max',type:'2 спальни, премиум',district:'Ваке',price:1400,status:'Свободно',agent:'David',owner:'База собственников',phone:'995XXXXXXXXX',notes:'Престижный дом рядом с McDonald’s, кафе и торговым центром.'},
    {id:uid(),address:'ул. Асатиани',type:'1 спальня',district:'Сололаки',price:780,status:'Сдано',agent:'Sergi',owner:'База собственников',phone:'995XXXXXXXXX',notes:'Внутренний двор, 2-й этаж.'},
    {id:uid(),address:'Батуми, Legenda',type:'Инвестиция / черный каркас',district:'Батуми',price:14000,status:'В работе',agent:'David',owner:'Застройщик',phone:'995XXXXXXXXX',notes:'20 этажей, 11 построено, вид на море/порт, паркинг $14 000.'}
  ],
  owners: [
    {id:uid(),name:'Собственник — Атонели',phone:'995XXXXXXXXX',object:'ул. Атонели',status:'Эксклюзив',last:'Перед публикацией обязателен Owner Information. Проверить дубликаты на SS.ge / MyHome.'},
    {id:uid(),name:'Собственник — Кипшидзе 6',phone:'995XXXXXXXXX',object:'Кипшидзе 6',status:'В работе',last:'Коммерческий объект: подтвердить реестр, фасад и финальную цену.'},
    {id:uid(),name:'Собственник — Аракишвили',phone:'995XXXXXXXXX',object:'ул. Аракишвили',status:'В работе',last:'Нужны реестр, кадастровый код и проверка порядка фото.'},
    {id:uid(),name:'Собственник — Outlook Ваке',phone:'995XXXXXXXXX',object:'Outlook Ваке',status:'Эксклюзив',last:'Премиум-объект. Использовать качественные фото и короткие рекламные тексты.'}
  ],
  clients: [
    ...
    {id:uid(),name:'Гость посуточной аренды',phone:'—',request:'Контроль заселения, локера, оплаты, Wi‑Fi и расходников',budget:900,status:'Показ',agent:'Nastya'}
  ],
  reports: [
    {id:uid(),type:'Общий отчет',agent:'Dima',target:'Фото / Показ / Стикеры / Звонки / Сделка',status:'Готово',date:new Date().toISOString().slice(0,10)},
    {id:uid(),type:'Ежедневный отчет',agent:'Mari',target:'Ежедневный контроль оператора',status:'Запланировано',date:new Date().toISOString().slice(0,10)},
    {id:uid(),type:'Фото с платформы',agent:'Sergi',target:'Уведомить топик + добавить адрес в базу собственников',status:'В работе',date:new Date().toISOString().slice(0,10)},
    {id:uid(),type:'Проверка объекта',agent:'Zurab',target:'Поиск собственника по телефону/адресу',status:'Запланировано',date:new Date().toISOString().slice(0,10)}
  ],
  agents: [
    ...
    {id:uid(),name:'Sergi',role:'Агент',deals:8,objects:24,level:'50%'},
    {id:uid(),name:'Рекрут',role:'Рекрут',deals:0,objects:3,level:'50%'}
  ],
  tasks: [
    ...
    {id:uid(),time:'15:00',title:'Порядок публикации: 9 фото + 1 видео + геолинк',status:'В работе',agent:'Агент'},
    {id:uid(),time:'17:00',title:'Предпросмотр договора: RU / GE / EN',status:'Запланировано',agent:'Irakli'}
  ]
};
...
function normalizeDb(data){
  ...
  if(Array.isArray(data.agents)){
    data.agents = data.agents.map(agent => ({...agent, objects: Number(agent.objects ?? agent.объекты ?? 0)}));
    data.agents.forEach(agent => delete agent.объекты);
  }
  return data;
}
...
function go(p){
  ...
  const titleEl=$('#title'); if(titleEl) titleEl.textContent=n[2];
  const subtitleEl=$('#subtitle'); if(subtitleEl) subtitleEl.textContent=subtitle(p);
  const sideEl=$('#side'); if(sideEl) sideEl.classList.remove('open');
  buildNav();
  render();
}
...
function dashboard(){
  ...
  return `<div class="dashboard-shell">
    ...
    <section class="panel telegram-panel" id="telegram-top10">
      <div class="panel-head telegram-panel-head">
        <div>
          <p class="eyebrow">Telegram-аналитика</p>
          <h3>Топ-10 квартир по просмотрам</h3>
        </div>
        <div class="telegram-panel-actions">
          <span class="telegram-top10-updated"></span>
          <button class="btn secondary" type="button" onclick="loadTelegramTop10()">Обновить</button>
        </div>
      </div>
      <div class="telegram-top10-content"><div class="telegram-state">Загружаем данные Telegram…</div></div>
    </section>
...
function render(){ buildNav(); let views={dashboard,properties,owners,clients,reports,contracts,agents,map,analytics,settings}; const contentEl=$('#content'); if(contentEl){ contentEl.innerHTML=views[page]?views[page]():developmentPage('Раздел', 'Содержимое временно недоступно'); } updateSideStats(); if(page==='dashboard'&&typeof window.loadTelegramTop10==='function') window.loadTelegramTop10(); }
...
function duplicateItem(key,id){ let old=db[key].find(x=>x.id===id); if(!old) return; db[key].unshift({...old,id:uid(),address:old.address?old.address+' копия':old.address,name:old.name?old.name+' копия':old.name}); save(); render(); toast('Скопировано'); }
...
function createReportFrom(type,target){ db.reports.unshift({id:uid(),date:new Date().toISOString().slice(0,10),type,agent:'',target,status:'Запланировано'}); save(); toast('Отчет создан'); go('reports'); }
...
const menuBtn=$('#menuBtn'); if(menuBtn) menuBtn.onclick=()=>$('#side')?.classList.toggle('open');
